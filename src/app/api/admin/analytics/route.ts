...
export async function GET() {
  try {
    ...
    const [
      ...
      prisma.utilisateur.count({ where: { centerId, role: "eleve", deletedAt: null, ghost: false } }),
      prisma.utilisateur.count({ where: { centerId, role: "eleve", createdAt: { gte: startOfMonth }, deletedAt: null, ghost: false } }),
      prisma.utilisateur.count({ where: { centerId, role: "eleve", createdAt: { gte: startOfLastMonth, lte: endOfLastMonth }, deletedAt: null, ghost: false } }),
      prisma.utilisateur.count({ where: { centerId, role: "prof", deletedAt: null, ghost: false } }),
